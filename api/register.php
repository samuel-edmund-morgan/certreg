...
// Expect JSON: {cid, v, h, course, grade, date, valid_until?, extra_info?}
...
try {
  if($v===2){
    ...
    $st = $pdo->prepare("INSERT INTO tokens (cid, version, h, course, grade, extra_info, issued_date, valid_until) VALUES (?,?,?,?,?,?,?,?)");
    $st->execute([$cid,$v,$h,$course,$grade,$extra,$date,$validUntil]);
  } else {
