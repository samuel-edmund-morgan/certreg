...
Нові lifecycle тести (`verify_lifecycle.spec.js`) перевіряють:
- Revoke → status зміна на "Відкликано" і відображення причини.
- Unrevoke → повернення статусу "Активний".
- Delete → повторна верифікація показує "не знайдено".

Для підвищення стабільності UI тесту revoke/unrevoke використовується прямий POST до `/api/revoke.php` та `/api/unrevoke.php` з вилученням CSRF токена з рядка таблиці (у деяких headless ситуаціях innerHTML може не містити повний `<form>`). Це не обходить бізнес‑логіку, бо бекенд застосовує ті ж перевірки (CSRF, існування токена, ідempotency).
...
Пер‑рядкові (inline) дії відкликання / відновлення реалізовані через AJAX (`assets/js/tokens_page.js`); тести можуть використовувати або клік по формі, або прямий fetch з FormData (як fallback). Якщо зміниться верстка клітинки статусу – оновіть селектори в `verify_lifecycle.spec.js`.
