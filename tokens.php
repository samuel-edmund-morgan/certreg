...
$allowedSorts = ['id', 'cid', 'version', 'extra_info', 'issued_date', 'created_at', 'status', 'lookup_count'];
...
$where = '';$params=[];$conds=[];
if($q!==''){
  $conds[] = "(cid LIKE :q OR extra_info LIKE :q)";
  $params[':q'] = "%$q%";
}
if($state==='active'){
...
$st = $pdo->prepare("SELECT cid, version, extra_info, issued_date, revoked_at, revoke_reason, created_at, lookup_count, last_lookup_at FROM tokens $where {$orderBy} LIMIT :lim OFFSET :off");
