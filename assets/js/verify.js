;(function(){
  ...
  if(!payload || (payload.v !== 3 && payload.v !== 4)) return;
  ...
  const awardOut = document.getElementById('awardOut');
  const awardSummary = document.getElementById('awardSummary');
  const awardHeadline = document.getElementById('awardHeadline');
  ...
  let awardTitle = payload.award_title || '';
  text(awardOut, awardTitle);
  ...
  fetch(statusUrl).then(r=>r.json()).then(js=>{
    ...
    awardTitle = js.award_title || awardTitle || 'Нагорода';
    if(awardOut){ awardOut.textContent = awardTitle; }
    if(awardHeadline){ awardHeadline.textContent = awardTitle; }
    ...
    if(js.revoked){
      ...
      const awardLine = `<p><strong>Нагорода:</strong> ${escapeHtml(awardTitle)}</p>`;
      ...
      existBox.innerHTML = `<p>Нагорода існує, але <strong class="text-danger">ВІДКЛИКАНА</strong>.</p>${awardLine}${dateLine}${reasonLine}`;
      if(ownForm) ownForm.style.display='none';
      return;
    }
    if(expired){
      existBox.className='alert alert-error';
      existBox.innerHTML=`<p>Реєстраційний номер існує, але строк дії минув.</p><p><strong>Нагорода:</strong> ${escapeHtml(awardTitle)}</p>`;
      if(ownForm) ownForm.style.display='block';
    } else {
      existBox.className='alert';
      const expiryInfo = js.valid_until ? (js.valid_until===INFINITE_SENTINEL ? ' (безстрокова)' : ' (чинна до '+js.valid_until+')') : '';
      existBox.innerHTML=`<p>Реєстраційний номер існує, нагорода чинна. INT ${short}${expiryInfo}</p><p><strong>Нагорода:</strong> ${escapeHtml(awardTitle)}</p>`;
      if(ownForm) ownForm.style.display='block';
    }
    if(!ownForm) return;
  ...
  function toAwardSentenceCase(rawTitle){ let s=String(rawTitle||'').normalize('NFC').trim(); if(!s) return s; const lower=s.toLocaleLowerCase('uk'); const idx=lower.search(/\p{L}/u); if(idx===-1) return lower; return lower.slice(0,idx)+lower[idx].toLocaleUpperCase('uk')+lower.slice(idx+1); }
      ...
      let matched=false;
      const awardCanon = version === 4 ? (payload.award_title || awardTitle || 'Нагорода') : null;
      for(const orgCandidate of orgCandidates){
        const can = version === 4
          ? `v4|${pib}|${orgCandidate}|${awardCanon}|${payload.cid}|${payload.date}|${vu}|${canonUrl}|${payload.extra||''}`
          : `v3|${pib}|${orgCandidate}|${payload.cid}|${payload.date}|${vu}|${canonUrl}|${payload.extra||''}`;
  ...
  if(matched){ const displayName = toNameCaseUk(raw); const displayAward = toAwardSentenceCase(awardTitle); ownResult.innerHTML='<div class="alert alert-ok" data-verdict="match">'+escapeHtml(displayAward)+' дійсно належить '+escapeHtml(displayName)+'.</div>'; }
